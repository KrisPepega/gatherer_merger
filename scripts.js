...
function lua_to_json(input) {
	...
	let diff
	do {
		// replace curlies around arrays with square brackets
		diff = str.length
		str = str.replace(
			/\{(((\n\t*)\t)\S.*(\2.*)*)\,\s--\s\[\d+\]\3\}/g,
			"[$1$3]"
...
function json_to_lua(input) {
	let str = input
	//console.log(str)
	str = str.replace(/("([0-9]*)"|("[a-z-\s-']*"))\s*:/g, "[$2$3] =")
	return str
}
...
function merger(master, mergee) {
	let lua1 = master
	let lua2 = mergee
	let acc_settings = lua1.replace(
		/[\s\S]*(?=(Gatherer_SavedSettings_AccountWide))/,
		""
	)
	let temp = json_to_lua(
		JSON.stringify(merge(lua_to_json(lua1), lua_to_json(lua2)), null, 4)
	)
...
function download_file(filename) {
	...
	element.setAttribute(
		"href",
		"data:text/plain;charset=utf-8," + encodeURIComponent(output_content)
	)
...
document
	...
	.addEventListener("click", () => (download_file("Gatherer.lua"), false))
